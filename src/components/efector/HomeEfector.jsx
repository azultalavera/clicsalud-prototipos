...
import {
  ...
  stepConnectorClasses, styled, Button, Stack, Tooltip, Fab, Tabs, Tab, Divider,
} from "@mui/material";
import {
...
const HomeEfector = () => {
  ...
  return (
    <Layout>
      <Box sx={{ position: "sticky", top: 64, zIndex: 10, bgcolor: "#fff", borderBottom: "1px solid #e0e0e0", mx: -4, mt: -4, mb: 4, px: 4 }}>
        <Tabs 
          value={currentPath.includes("observartramite") ? `${baseRoute}/observartramite` : currentPath.includes("actainspeccion") ? `${baseRoute}/actainspeccion` : `${baseRoute}/vertramite`}
          textColor="primary" 
          indicatorColor="primary"
        >
          <Tab label="VER TRÁMITE" value={`${baseRoute}/vertramite`} component={Link} to={`${baseRoute}/vertramite`} sx={{ fontWeight: "bold" }} />
          <Tab label="OBSERVAR TRÁMITE" value={`${baseRoute}/observartramite`} component={Link} to={`${baseRoute}/observartramite`} sx={{ fontWeight: "bold" }} />
          <Tab label="ACTA DE INSPECCIÓN" value={`${baseRoute}/actainspeccion`} component={Link} to={`${baseRoute}/actainspeccion`} sx={{ fontWeight: "bold" }} />
        </Tabs>
      </Box>
        ...
        <Box sx={{ p: 4, backgroundColor: "white" }}>
          ...
          <Box sx={{ minHeight: "450px" }}>
            <Routes>
              ...
              <Route path="servicios" element={<ServicesStep selectedServices={selectedServices} setSelectedServices={setSelectedServices} infraSelection={infraSelection} setInfraSelection={setInfraSelection} onValidationChange={setIsServiceValid} />} />
              ...
              <Route path="equipamientos" element={<Equipamientos selectedServices={selectedServices} infraSelection={infraSelection} equiposCargados={equiposCargados} setEquiposCargados={setEquiposCargados} onValidationChange={setIsEquipamientoValid} />} />
              <Route path="observartramite" element={<ObservarTramite selectedServices={selectedServices} infraSelection={infraSelection} equiposCargados={equiposCargados} />} />
          ...
          {!["observartramite", "actainspeccion"].some(p => currentPath.includes(p)) && (
            <Box sx={{ mt: 4, pt: 3, borderTop: "1px solid #eee", display: "flex", justifyContent: "space-between" }}>
              ...
              <Stack direction="row" spacing={2}>
                ...
                <Button variant="outlined" startIcon={<SaveIcon />} onClick={handleQAFill}>GUARDAR</Button>
                <Button 
                  variant="contained" 
                  endIcon={<SendIcon />} 
                  onClick={handleNext} 
                  disabled={(activeStep === 3 && !isServiceValid) || (activeStep === 6 && !isEquipamientoValid) || activeStep === steps.length - 1} 
