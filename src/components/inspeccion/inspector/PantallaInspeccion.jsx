...
const PantallaInspeccion = ({
  ...
  const efectorResponses = {
    "Quirófanos": { msg: "Se adjunta plano rectificado con 11 quirófanos.", file: "plano_final.pdf" },
    "Plan de Evacuación": { msg: "Nuevo plan aprobado por bomberos.", file: "bomberos.pdf" }
  };
    ...
    >
      ...
      {datosGeneralesSrv && (
        ...
        >
          ...
          >
            {currentActa === 1 ? (
              <RevisionActa section="GENERAL" efectorResponses={efectorResponses} />
            ) : (
      ...
      >
        ...
        <AccordionDetails sx={{ px: { xs: 2, sm: 3 }, py: 2, bgcolor: "#ffffff", borderTop: "1px solid #e2e8f0" }}>
          {currentActa === 1 ? (
            <RevisionActa section="TRAMITE" efectorResponses={efectorResponses} />
          ) : (
            <>
              <Box
                sx={{
              ...
              <Typography sx={{ color: "#94a3b8", fontStyle: "italic", fontSize: "0.9rem" }}>No hay observaciones pendientes.</Typography>
                ...
                <Box component="ul" sx={{ m: 0, pl: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {obsDatosGenerales.map((obs, idx) => (
                        <Box component="li" key={idx} sx={{ color: obs.type === 'ERROR' ? '#ef4444' : '#475569', fontSize: '0.95rem' }}>
    ...
    >
      <Box sx={{ flexGrow: 1, minWidth: 0 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          {onOpenObs && field.type !== "boolean" && field.type !== "checkbox" && (
            <IconButton
              size="small"
